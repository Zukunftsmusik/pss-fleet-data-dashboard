...
from app.models.enums import ParameterInterval, ParameterOnMissing
...
async def from_to_date_parameters(
    from_date: Annotated[
        ...
        Query(
            alias="fromDate",
            ge=CONSTANTS.pss_start_date,  # pyright: ignore[reportArgumentType]
            ...
            examples=[datetime(2019, 11, 30, 23, 59, 0)],  # noqa: DTZ001
        ),
    ] = None,
    to_date: Annotated[
        datetime | None,
        Query(
            ...
            examples=[datetime.now().replace(microsecond=0)],  # noqa: DTZ005
        ),
    ] = None,
) -> DatetimeFilter:
    """
    ...
    if from_date is not None:
        from_date = cast(datetime, utils.remove_timezone(utils.localize_to_utc(from_date))).replace(microsecond=0)

    if to_date is not None:
        to_date = cast(datetime, utils.remove_timezone(utils.localize_to_utc(to_date))).replace(microsecond=0)
...
async def list_filter_parameters(
    ...
    desc: Annotated[bool | None, Query(description="Return the results in descending order by timestamp.", examples=[False])] = None,
) -> ListFilter:
    """
...
async def skip_take_parameters(
    skip: Annotated[int | None, Query(ge=0, description="Skip this number of results from the result set.", examples=[0])] = None,
    take: Annotated[int | None, Query(ge=1, le=100, description="Limit the number of results returned.", examples=[100])] = None,
) -> SkipTakeFilter:
    """
